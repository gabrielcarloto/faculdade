...
Object.entries(columnsElements).forEach(([key, th]) => {
    ...
    button.addEventListener('click', () => {
        ...
        sortTable(key, nextSort === 'ascending');
...
function getColumnEl(name) {
    const el = document.querySelector(`th[data-col="${name}"]`);
    if (!el)
...
function updateRowSpan(rows) {
    const columnsToCheck = ['artist', 'year', 'sales'];
    for (const column of columnsToCheck) {
        ...
        while (i < rows.length) {
            ...
            for (let j = i + 1; j < rows.length; j++) {
                ...
                if (nextRowInfo.data !== rowInfo.data)
                    break;
                span++;
...
function sortTable(columnKey, ascending = true) {
